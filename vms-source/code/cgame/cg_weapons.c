...
static void CG_GrenadeTrail( centity_t *ent, const weaponInfo_t *wi ) {
	//CG_RocketTrail( ent, wi );  // We now use separate effects  Shafe - Trep
	// Shafe - Trep all of this is just copied from CG_RocketTrail with a changed shader... This can be consolidated a bit better... later
	int		step;
	vec3_t	origin, lastPos;
	int		t;
	int		startTime, contents;
...
void CG_AddViewWeapon( playerState_t *ps ) {
	...
	//	vec3_t		origin;
		...
		if ( cg.predictedPlayerState.eFlags & EF_FIRING ) {
			...
			// Shafe - Trep - We dont have a ligning gun anymore so the next 3 lines aren't needed
			//VectorCopy( cg.refdef.vieworg, origin );
