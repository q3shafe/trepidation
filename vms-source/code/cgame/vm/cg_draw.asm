...
;1971:	if ( trace.entityNum >= MAX_CLIENTS ) {
ADDRLP4 0+52
INDIRI4
CNSTI4 64
LTI4 $948
...
;1973:	}
;1974:
;1975:	// if the player is in fog, don't show it
;1976:	content = trap_CM_PointContents( trace.endpos, 0 );
ADDRLP4 0+12
ARGP4
CNSTI4 0
ARGI4
...
;1979:	}
;1980:
;1981:	// if the player is invisible, don't show it
;1982:	if ( cg_entities[ trace.entityNum ].currentState.powerups & ( 1 << PW_INVIS ) ) {
CNSTI4 732
ADDRLP4 0+52
INDIRI4
MULI4
...
;1984:	}
;1985:
;1986:	// update the fade timer
;1987:	cg.crosshairClientNum = trace.entityNum;
ADDRGP4 cg+124412
ADDRLP4 0+52
INDIRI4
ASGNI4
line 1988
;1988:	cg.crosshairClientTime = cg.time;
ADDRGP4 cg+124416
ADDRGP4 cg+107604
INDIRI4
ASGNI4
...
;1992:/*
;1993:=====================
;1994:CG_DrawCrosshairNames
;1995:=====================
;1996:*/
;1997:static void CG_DrawCrosshairNames( void ) {
line 2002
;1998:	float		*color;
;1999:	char		*name;
;2000:	float		w;
;2001:
;2002:	if ( !cg_drawCrosshair.integer ) {
ADDRGP4 cg_drawCrosshair+12
INDIRI4
CNSTI4 0
NEI4 $963
...
line 2005
;2004:	}
;2005:	if ( !cg_drawCrosshairNames.integer ) {
...
line 2008
;2007:	}
;2008:	if ( cg.renderingThirdPerson ) {
ADDRGP4 cg+107628
INDIRI4
CNSTI4 0
EQI4 $969
...
;2010:	}
;2011:
;2012:	// scan the known entities to see if the crosshair is sighted on one
;2013:	CG_ScanForCrosshairEntity();
ADDRGP4 CG_ScanForCrosshairEntity
CALLV
pop
line 2016
;2014:
;2015:	// draw the name of the player being looked at
;2016:	color = CG_FadeColor( cg.crosshairClientTime, 1000 );
ADDRGP4 cg+124416
INDIRI4
ARGI4
CNSTI4 1000
...
;2020:	}
;2021:
;2022:	name = cgs.clientinfo[ cg.crosshairClientNum ].name;
ADDRLP4 4
CNSTI4 1716
ADDRGP4 cg+124412
INDIRI4
...
;2024:	color[3] *= 0.5f;
;2025:	w = CG_Text_Width(name, 0.3f, 0);
;2026:	CG_Text_Paint( 320 - w / 2, 190, 0.3f, color, name, 0, 0, ITEM_TEXTSTYLE_SHADOWED);
;2027:#else
;2028:	w = CG_DrawStrlen( name ) * BIGCHAR_WIDTH;
ADDRLP4 4
INDIRP4
ARGP4
ADDRLP4 16
...
ASGNF4
line 2029
;2029:	CG_DrawBigString( 320 - w / 2, 170, name, color[3] * 0.5f );
CNSTF4 1134559232
ADDRLP4 8
INDIRF4
CNSTF4 1073741824
...
;2037:/*
;2038:=================
;2039:CG_DrawSpectator
;2040:=================
;2041:*/
;2042:static void CG_DrawSpectator(void) {
line 2043
;2043:	CG_DrawBigString(320 - 9 * 8, 440, "SPECTATOR", 1.0F);
CNSTI4 248
ARGI4
CNSTI4 440
ARGI4
...
pop
line 2044
;2044:	if ( cgs.gametype == GT_TOURNAMENT ) {
ADDRGP4 cgs+31456
INDIRI4
CNSTI4 1
NEI4 $980
line 2045
;2045:		CG_DrawBigString(320 - 15 * 8, 460, "waiting to play", 1.0F);
CNSTI4 200
ARGI4
CNSTI4 460
ARGI4
...
LABELV $980
line 2047
;2047:	else if ( cgs.gametype >= GT_TEAM ) {
ADDRGP4 cgs+31456
INDIRI4
CNSTI4 3
LTI4 $984
line 2048
;2048:		CG_DrawBigString(320 - 39 * 8, 460, "press ESC and use the JOIN menu to play", 1.0F);
CNSTI4 8
ARGI4
CNSTI4 460
ARGI4
...
;2055:=================
;2056:*/
;2057:static void CG_DrawVote(void) {
line 2061
;2058:	char	*s;
;2059:	int		sec;
;2060:
;2061:	if ( !cgs.voteTime ) {
ADDRGP4 cgs+31676
INDIRI4
CNSTI4 0
NEI4 $989
...
;2063:	}
;2064:
;2065:	// play a talk beep whenever it is modified
;2066:	if ( cgs.voteModified ) {
ADDRGP4 cgs+31688
INDIRI4
CNSTI4 0
EQI4 $992
line 2067
;2067:		cgs.voteModified = qfalse;
ADDRGP4 cgs+31688
CNSTI4 0
ASGNI4
line 2068
...
line 2071
;2070:
;2071:	sec = ( VOTE_TIME - ( cg.time - cgs.voteTime ) ) / 1000;
ADDRLP4 0
CNSTI4 30000
ADDRGP4 cg+107604
INDIRI4
...
;2076:	s = va("VOTE(%i):%s yes:%i no:%i", sec, cgs.voteString, cgs.voteYes, cgs.voteNo);
;2077:	CG_DrawSmallString( 0, 58, s, 1.0F );
;2078:	s = "or press ESC then click Vote";
;2079:	CG_DrawSmallString( 0, 58 + SMALLCHAR_HEIGHT + 2, s, 1.0F );
;2080:#else
;2081:	s = va("VOTE(%i):%s yes:%i no:%i", sec, cgs.voteString, cgs.voteYes, cgs.voteNo );
ADDRGP4 $1002
ARGP4
ADDRLP4 0
INDIRI4
...
ASGNP4
line 2082
;2082:	CG_DrawSmallString( 0, 58, s, 1.0F );
CNSTI4 0
ARGI4
CNSTI4 58
ARGI4
...
;2086:/*
;2087:=================
;2088:CG_DrawTeamVote
;2089:=================
;2090:*/
;2091:static void CG_DrawTeamVote(void) {
line 2095
;2092:	char	*s;
;2093:	int		sec, cs_offset;
;2094:
;2095:	if ( cgs.clientinfo->team == TEAM_RED )
...
LABELV $1007
line 2097
;2097:	else if ( cgs.clientinfo->team == TEAM_BLUE )
ADDRGP4 cgs+40972+68
INDIRI4
CNSTI4 2
NEI4 $1006
...
line 2102
;2101:
;2102:	if ( !cgs.teamVoteTime[cs_offset] ) {
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
...
;2107:	if ( cgs.teamVoteModified[cs_offset] ) {
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
...
EQI4 $1018
line 2108
;2108:		cgs.teamVoteModified[cs_offset] = qfalse;
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
...
line 2112
;2111:
;2112:	sec = ( VOTE_TIME - ( cg.time - cgs.teamVoteTime[cs_offset] ) ) / 1000;
ADDRLP4 4
CNSTI4 30000
ADDRGP4 cg+107604
INDIRI4
...
LABELV $1026
line 2116
;2116:	s = va("TEAMVOTE(%i):%s yes:%i no:%i", sec, cgs.teamVoteString[cs_offset],
ADDRGP4 $1028
ARGP4
ADDRLP4 4
INDIRI4
...
ASGNP4
line 2118
;2117:							cgs.teamVoteYes[cs_offset], cgs.teamVoteNo[cs_offset] );
;2118:	CG_DrawSmallString( 0, 90, s, 1.0F );
CNSTI4 0
ARGI4
CNSTI4 90
ARGI4
...
;2120:
;2121:
;2122:static qboolean CG_DrawScoreboard() {
line 2187
;2123:#ifdef MISSIONPACK
;2124:	static qboolean firstTime = qtrue;
;2125:	float fade, *fadeColor;
;2126:
;2127:	if (menuScoreboard) {
;2128:		menuScoreboard->window.flags &= ~WINDOW_FORCED;
;2129:	}
;2130:	if (cg_paused.integer) {
;2131:		cg.deferredPlayerLoading = 0;
;2132:		firstTime = qtrue;
;2133:		return qfalse;
;2134:	}
;2135:
;2136:	// should never happen in Team Arena
;2137:	if (cgs.gametype == GT_SINGLE_PLAYER && cg.predictedPlayerState.pm_type == PM_INTERMISSION ) {
;2138:		cg.deferredPlayerLoading = 0;
;2139:		firstTime = qtrue;
;2140:		return qfalse;
;2141:	}
;2142:
;2143:	// don't draw scoreboard during death while warmup up
;2144:	if ( cg.warmup && !cg.showScores ) {
;2145:		return qfalse;
;2146:	}
;2147:
;2148:	if ( cg.showScores || cg.predictedPlayerState.pm_type == PM_DEAD || cg.predictedPlayerState.pm_type == PM_INTERMISSION ) {
;2149:		fade = 1.0;
;2150:		fadeColor = colorWhite;
;2151:	} else {
;2152:		fadeColor = CG_FadeColor( cg.scoreFadeTime, FADE_TIME );
;2153:		if ( !fadeColor ) {
;2154:			// next time scoreboard comes up, don't print killer
;2155:			cg.deferredPlayerLoading = 0;
;2156:			cg.killerName[0] = 0;
;2157:			firstTime = qtrue;
;2158:			return qfalse;
;2159:		}
;2160:		fade = *fadeColor;
;2161:	}																					  
;2162:
;2163:
;2164:	if (menuScoreboard == NULL) {
;2165:		if ( cgs.gametype >= GT_TEAM ) {
;2166:			menuScoreboard = Menus_FindByName("teamscore_menu");
;2167:		} else {
;2168:			menuScoreboard = Menus_FindByName("score_menu");
;2169:		}
;2170:	}
;2171:
;2172:	if (menuScoreboard) {
;2173:		if (firstTime) {
;2174:			CG_SetScoreSelection(menuScoreboard);
;2175:			firstTime = qfalse;
;2176:		}
;2177:		Menu_Paint(menuScoreboard, qtrue);
;2178:	}
;2179:
;2180:	// load any models that have been deferred
;2181:	if ( ++cg.deferredPlayerLoading > 10 ) {
;2182:		CG_LoadDeferredPlayers();
;2183:	}
;2184:
;2185:	return qtrue;
;2186:#else
;2187:	return CG_DrawOldScoreboard();
ADDRLP4 0
...
;2191:/*
;2192:=================
;2193:CG_DrawIntermission
;2194:=================
;2195:*/
;2196:static void CG_DrawIntermission( void ) {
line 2204
;2197://	int key;
;2198:#ifdef MISSIONPACK
;2199:	//if (cg_singlePlayer.integer) {
;2200:	//	CG_DrawCenterString();
;2201:	//	return;
;2202:	//}
;2203:#else
;2204:	if ( cgs.gametype == GT_SINGLE_PLAYER ) {
ADDRGP4 cgs+31456
INDIRI4
CNSTI4 2
NEI4 $1034
line 2205
;2205:		CG_DrawCenterString();
...
;2207:	}
;2208:#endif
;2209:	cg.scoreFadeTime = cg.time;
ADDRGP4 cg+114340
ADDRGP4 cg+107604
INDIRI4
ASGNI4
line 2210
;2210:	cg.scoreBoardShowing = CG_DrawScoreboard();
ADDRLP4 0
...
;2216:=================
;2217:*/
;2218:static qboolean CG_DrawFollow( void ) {
line 2223
;2219:	float		x;
;2220:	vec4_t		color;
;2221:	const char	*name;
;2222:
;2223:	if ( !(cg.snap->ps.pm_flags & PMF_FOLLOW) ) {
ADDRGP4 cg+36
INDIRP4
CNSTI4 56
ADDP4
...
;2230:
;2231:
;2232:	CG_DrawBigString( 320 - 9 * 8, 24, "following", 1.0F );
CNSTI4 248
ARGI4
CNSTI4 24
ARGI4
...
line 2234
;2233:
;2234:	name = cgs.clientinfo[ cg.snap->ps.clientNum ].name;
ADDRLP4 16
CNSTI4 1716
ADDRGP4 cg+36
INDIRP4
...
line 2236
;2235:
;2236:	x = 0.5 * ( 640 - GIANT_WIDTH * CG_DrawStrlen( name ) );
ADDRLP4 16
INDIRP4
ARGP4
ADDRLP4 24
...
line 2238
;2237:
;2238:	CG_DrawStringExt( x, 40, name, color, qtrue, qtrue, GIANT_WIDTH, GIANT_HEIGHT, 0 );
ADDRLP4 20
INDIRF4
CVFI4 4
ARGI4
...
;2245:/*
;2246:=================
;2247:CG_DrawAmmoWarning
;2248:=================
;2249:*/
;2250:static void CG_DrawAmmoWarning( void ) {
line 2254
;2251:	const char	*s;
;2252:	int			w;
;2253:
;2254:	if ( cg_drawAmmoWarning.integer == 0 ) {
...
LABELV $1059
line 2267
;2267:	w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
ADDRLP4 0
INDIRP4
ARGP4
ADDRLP4 8
...
ASGNI4
line 2268
;2268:	CG_DrawSmallString(320 - w / 2, 64, s, 1.0F);
CNSTI4 320
ADDRLP4 4
INDIRI4
CNSTI4 2
...
;2270:
;2271:
;2272:#ifdef MISSIONPACK
;2273:/*
;2274:=================
;2275:CG_DrawProxWarning
;2276:=================
;2277:*/
;2278:static void CG_DrawProxWarning( void ) {
;2279:	char s [32];
;2280:	int			w;
;2281:  static int proxTime;
;2282:  static int proxCounter;
;2283:  static int proxTick;
;2284:
;2285:	if( !(cg.snap->ps.eFlags & EF_TICKING ) ) {
;2286:    proxTime = 0;
;2287:		return;
;2288:	}
;2289:
;2290:  if (proxTime == 0) {
;2291:    proxTime = cg.time + 5000;
;2292:    proxCounter = 5;
;2293:    proxTick = 0;
;2294:  }
;2295:
;2296:  if (cg.time > proxTime) {
;2297:    proxTick = proxCounter--;
;2298:    proxTime = cg.time + 1000;
;2299:  }
;2300:
;2301:  if (proxTick != 0) {
;2302:    Com_sprintf(s, sizeof(s), "INTERNAL COMBUSTION IN: %i", proxTick);
;2303:  } else {
;2304:    Com_sprintf(s, sizeof(s), "YOU HAVE BEEN MINED");
;2305:  }
;2306:
;2307:	w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
;2308:	CG_DrawBigStringColor( 320 - w / 2, 64 + BIGCHAR_HEIGHT, s, g_color_table[ColorIndex(COLOR_RED)] );
;2309:}
;2310:#endif
;2311:
;2312:
...
;2316:=================
;2317:*/
;2318:static void CG_DrawWarmup( void ) {
line 2327
;2319:	int			w;
;2320:	int			sec;
;2321:	int			i;
;2322:	float scale;
;2323:	clientInfo_t	*ci1, *ci2;
;2324:	int			cw;
;2325:	const char	*s;
;2326:
;2327:	sec = cg.warmup;
...
GEI4 $1067
line 2333
;2333:		s = "Waiting for players";		
ADDRLP4 8
ADDRGP4 $1069
ASGNP4
line 2334
;2334:		w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
ADDRLP4 8
INDIRP4
ARGP4
ADDRLP4 32
...
ASGNI4
line 2335
;2335:		CG_DrawBigString(320 - w / 2, 24, s, 1.0F);
CNSTI4 320
ADDRLP4 16
INDIRI4
CNSTI4 2
...
;2338:	}
;2339:
;2340:	if (cgs.gametype == GT_TOURNAMENT) {
ADDRGP4 cgs+31456
INDIRI4
CNSTI4 1
NEI4 $1071
line 2342
;2341:		// find the two active players
;2342:		ci1 = NULL;
ADDRLP4 20
CNSTP4 0
ASGNP4
...
ASGNP4
line 2344
;2344:		for ( i = 0 ; i < cgs.maxclients ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $1077
...
LABELV $1074
line 2345
;2345:			if ( cgs.clientinfo[i].infoValid && cgs.clientinfo[i].team == TEAM_FREE ) {
ADDRLP4 32
CNSTI4 1716
ADDRLP4 0
INDIRI4
...
EQU4 $1072
line 2355
;2355:			s = va( "%s vs %s", ci1->name, ci2->name );
ADDRGP4 $1090
ARGP4
ADDRLP4 36
CNSTI4 4
...
ASGNP4
line 2360
;2356:#ifdef MISSIONPACK
;2357:			w = CG_Text_Width(s, 0.6f, 0);
;2358:			CG_Text_Paint(320 - w / 2, 60, 0.6f, colorWhite, s, 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE);
;2359:#else
;2360:			w = CG_DrawStrlen( s );
ADDRLP4 8
INDIRP4
ARGP4
ADDRLP4 44
...
LABELV $1092
line 2366
;2366:			CG_DrawStringExt( 320 - w * cw/2, 20,s, colorWhite, 
CNSTI4 320
ADDRLP4 16
INDIRI4
ADDRLP4 12
...
pop
line 2370
;2367:					qfalse, qtrue, cw, (int)(cw * 1.5f), 0 );
;2368:			
;2369:#endif
;2370:		}
line 2371
...
ASGNP4
line 2374
;2374:		} else if ( cgs.gametype == GT_FFA && cgs.g_GameMode == 0) {
ADDRGP4 $1094
JUMPV
LABELV $1093
ADDRLP4 32
...
ASGNP4
line 2378
;2378:		} else if ( cgs.gametype == GT_TEAM ) {
ADDRGP4 $1103
JUMPV
LABELV $1102
ADDRGP4 cgs+31456
...
NEI4 $1106
line 2379
;2379:			s = "Team Deathmatch";
ADDRLP4 8
ADDRGP4 $1109
ASGNP4
line 2380
;2380:		} else if ( cgs.gametype == GT_CTF ) {
ADDRGP4 $1107
JUMPV
LABELV $1106
ADDRGP4 cgs+31456
...
NEI4 $1110
line 2381
;2381:			s = "Capture the Flag";
ADDRLP4 8
ADDRGP4 $1113
ASGNP4
line 2390
;2382:#ifdef MISSIONPACK
;2383:		} else if ( cgs.gametype == GT_1FCTF ) {
;2384:			s = "One Flag CTF";
;2385:		} else if ( cgs.gametype == GT_OBELISK ) {
;2386:			s = "Overload";
;2387:		} else if ( cgs.gametype == GT_HARVESTER ) {
;2388:			s = "Harvester";
;2389:#endif
;2390:		} else {
ADDRGP4 $1111
...
;2394:		w = CG_Text_Width(s, 0.6f, 0);
;2395:		CG_Text_Paint(320 - w / 2, 90, 0.6f, colorWhite, s, 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE);
;2396:#else
;2397:		w = CG_DrawStrlen( s );
ADDRLP4 8
INDIRP4
ARGP4
ADDRLP4 36
...
LABELV $1116
line 2403
;2403:		CG_DrawStringExt( 320 - w * cw/2, 25,s, colorWhite, 
CNSTI4 320
ADDRLP4 16
INDIRI4
ADDRLP4 12
...
pop
line 2406
;2404:				qfalse, qtrue, cw, (int)(cw * 1.1f), 0 );
;2405:#endif
;2406:	}
LABELV $1072
line 2408
;2407:
;2408:	sec = ( sec - cg.time ) / 1000;
ADDRLP4 4
ADDRLP4 4
INDIRI4
ADDRGP4 cg+107604
...
;2415:	{
line 2416
;2416:		s = va( "You Have %i Seconds To Join This Round", sec + 1 );
ADDRGP4 $1124
ARGP4
ADDRLP4 4
INDIRI4
...
line 2427
;2426:		case 0:
;2427:			trap_S_StartLocalSound( cgs.media.count1Sound, CHAN_ANNOUNCER );
...
line 2430
;2429:		case 1:
;2430:			trap_S_StartLocalSound( cgs.media.count2Sound, CHAN_ANNOUNCER );
...
line 2433
;2432:		case 2:
;2433:			trap_S_StartLocalSound( cgs.media.count3Sound, CHAN_ANNOUNCER );
...
;2460:		w = CG_Text_Width(s, scale, 0);
;2461:		CG_Text_Paint(320 - w / 2, 125, scale, colorWhite, s, 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE);
;2462:#else
;2463:	w = CG_DrawStrlen( s );
ADDRLP4 8
INDIRP4
ARGP4
ADDRLP4 36
...
ASGNI4
line 2464
;2464:	CG_DrawStringExt( 320 - w * cw/2, 70, s, colorWhite, 
CNSTI4 320
ADDRLP4 16
INDIRI4
ADDRLP4 12
...
pop
line 2467
;2465:			qfalse, qtrue, cw, (int)(cw * 1.5), 0 );
;2466:#endif
;2467:}
LABELV $1063
...
;2468:
;2469://==================================================================================
;2470:#ifdef MISSIONPACK
;2471:/* 
;2472:=================
;2473:CG_DrawTimedMenus
;2474:=================
;2475:*/
;2476:void CG_DrawTimedMenus() {
;2477:	if (cg.voiceTime) {
;2478:		int t = cg.time - cg.voiceTime;
;2479:		if ( t > 2500 ) {
;2480:			Menus_CloseByName("voiceMenu");
;2481:			trap_Cvar_Set("cl_conXOffset", "0");
;2482:			cg.voiceTime = 0;
;2483:		}
;2484:	}
;2485:}
...
;2490:=================
;2491:*/
;2492:static void CG_Draw2D( void ) {
line 2499
;2493:#ifdef MISSIONPACK
;2494:	if (cgs.orderPending && cg.time > cgs.orderTime) {
;2495:		CG_CheckOrderPending();
;2496:	}
;2497:#endif
;2498:	// if we are taking a levelshot for the menu, don't draw anything
;2499:	if ( cg.levelShot ) {
ADDRGP4 cg+12
INDIRI4
CNSTI4 0
EQI4 $1148
...
;2505:	}
;2506:
;2507:	if ( cg.snap->ps.pm_type == PM_INTERMISSION ) {
ADDRGP4 cg+36
INDIRP4
CNSTI4 48
ADDP4
...
;2511:
;2512:/*
;2513:	if (cg.cameraMode) {
;2514:		return;
;2515:	}
;2516:*/
;2517:	if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR ) {
ADDRGP4 cg+36
INDIRP4
CNSTI4 304
ADDP4
...
NEI4 $1157
line 2518
;2518:		CG_DrawSpectator();
ADDRGP4 CG_DrawSpectator
CALLV
pop
line 2519
;2519:		CG_DrawCrosshair();
ADDRGP4 CG_DrawCrosshair
CALLV
pop
line 2520
;2520:		CG_DrawCrosshairNames();
...
LABELV $1157
line 2523
;2522:		// don't draw any status if dead or the scoreboard is being explicitly shown
;2523:		if ( !cg.showScores && cg.snap->ps.stats[STAT_HEALTH] > 0 ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 cg+114332
...
line 2531
;2524:
;2525:#ifdef MISSIONPACK
;2526:			if ( cg_drawStatus.integer ) {
;2527:				Menu_PaintAll();
;2528:				CG_DrawTimedMenus();
;2529:			}
;2530:#else
;2531:			CG_DrawStatusBar();
...
;2532:#endif
;2533:      
;2534:			CG_DrawAmmoWarning();
ADDRGP4 CG_DrawAmmoWarning
CALLV
pop
line 2539
;2535:
;2536:#ifdef MISSIONPACK
;2537:			CG_DrawProxWarning();
;2538:#endif      
;2539:			CG_DrawCrosshair();
ADDRGP4 CG_DrawCrosshair
CALLV
pop
line 2540
;2540:			CG_DrawCrosshairNames();
ADDRGP4 CG_DrawCrosshairNames
CALLV
pop
line 2541
;2541:			CG_DrawWeaponSelect();
ADDRGP4 CG_DrawWeaponSelect
CALLV
pop
line 2544
;2542:
;2543:#ifndef MISSIONPACK
;2544:			CG_DrawHoldableItem();
ADDRGP4 CG_DrawHoldableItem
CALLV
pop
line 2548
;2545:#else
;2546:			//CG_DrawPersistantPowerup();
;2547:#endif
;2548:			CG_DrawReward();
...
pop
line 2549
;2549:			CG_DrawScanner();  // Shafe - Trep - Radar
...
pop
line 2560
;2560:	CG_DrawTeamVote();
ADDRGP4 CG_DrawTeamVote
CALLV
pop
line 2562
;2561:
;2562:	CG_DrawLagometer();
...
;2565:	if (!cg_paused.integer) {
;2566:		CG_DrawUpperRight();
;2567:	}
;2568:#else
;2569:	CG_DrawUpperRight();
...
;2570:#endif
;2571:
;2572:#ifndef MISSIONPACK
;2573:	CG_DrawLowerRight();
ADDRGP4 CG_DrawLowerRight
CALLV
pop
line 2574
;2574:	CG_DrawLowerLeft();
...
line 2582
;2580:
;2581:	// don't draw center string if scoreboard is up
;2582:	cg.scoreBoardShowing = CG_DrawScoreboard();
ADDRLP4 4
ADDRGP4 CG_DrawScoreboard
CALLI4
ASGNI4
...
ASGNI4
line 2583
;2583:	if ( !cg.scoreBoardShowing) {
ADDRGP4 cg+114336
INDIRI4
CNSTI4 0
NEI4 $1170
line 2584
;2584:		CG_DrawCenterString();
ADDRGP4 CG_DrawCenterString
CALLV
pop
line 2585
...
;2587:
;2588:
;2589:static void CG_DrawTourneyScoreboard() {
line 2592
;2590:#ifdef MISSIONPACK
;2591:#else
;2592:	CG_DrawOldTourneyScoreboard();
...
;2598:CG_DrawActive
;2599:
;2600:Perform all drawing needed to completely fill the screen
;2601:=====================
;2602:*/
;2603:void CG_DrawActive( stereoFrame_t stereoView ) {
line 2608
;2604:	float		separation;
;2605:	vec3_t		baseOrg;
;2606:
;2607:	// optionally draw the info screen instead
;2608:	if ( !cg.snap ) {
ADDRGP4 cg+36
INDIRP4
CVPU4 4
...
;2611:	}
;2612:
;2613:	// optionally draw the tournement scoreboard instead
...
EQI4 $1178
line 2615
;2615:		( cg.snap->ps.pm_flags & PMF_SCOREBOARD ) ) {
line 2616
;2616:		CG_DrawTourneyScoreboard();
...
LABELV $1186
line 2625
;2624:	case STEREO_LEFT:
;2625:		separation = -cg_stereoSeparation.value / 2;
ADDRLP4 0
...
LABELV $1188
line 2628
;2627:	case STEREO_RIGHT:
;2628:		separation = cg_stereoSeparation.value / 2;
ADDRLP4 0
ADDRGP4 cg_stereoSeparation+8
INDIRF4
CNSTF4 1073741824
...
ASGNF4
line 2632
;2632:		CG_Error( "CG_DrawActive: Undefined stereoView" );
ADDRGP4 $1190
ARGP4
ADDRGP4 CG_Error
CALLV
...
;2634:
;2635:
;2636:	// clear around the rendered view if sized down
;2637:	CG_TileClear();
...
line 2640
;2638:
;2639:	// offset vieworg appropriately if we're doing stereo separation
;2640:	VectorCopy( cg.refdef.vieworg, baseOrg );
ADDRLP4 4
ADDRGP4 cg+109048+24
INDIRB
ASGNB 12
line 2641
;2641:	if ( separation != 0 ) {
ADDRLP4 0
INDIRF4
CNSTF4 0
EQF4 $1193
line 2642
;2642:		VectorMA( cg.refdef.vieworg, -separation, cg.refdef.viewaxis[1], cg.refdef.vieworg );
ADDRGP4 cg+109048+24
ADDRGP4 cg+109048+24
INDIRF4
...
;2644:
;2645:	// draw 3D view
;2646:	trap_R_RenderScene( &cg.refdef );
ADDRGP4 cg+109048
ARGP4
ADDRGP4 trap_R_RenderScene
CALLV
...
line 2649
;2647:
;2648:	// restore original viewpoint if running stereo
;2649:	if ( separation != 0 ) {
ADDRLP4 0
INDIRF4
CNSTF4 0
EQF4 $1223
line 2650
;2650:		VectorCopy( baseOrg, cg.refdef.vieworg );
ADDRGP4 cg+109048+24
ADDRLP4 4
INDIRB
ASGNB 12
...
line 2654
;2652:
;2653:	// draw status bar and other floating elements
;2654: 	CG_Draw2D();
ADDRGP4 CG_Draw2D
CALLV
pop
