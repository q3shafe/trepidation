...
static void CG_Obituary( entityState_t *ent ) {
	...
	if ( attacker != ENTITYNUM_WORLD ) {
		...
		case MOD_TELEFRAG:
			...
			break;
		case MOD_ALTFLAMER:		// Shafe - Trep - Flame Thrower Alt
...
//		CG_Printf( "EV_JUMP_PAD w/effect #%i\n", es->eventParm );
	...
	case EV_TAUNT:
		DEBUGNAME("EV_TAUNT");
		if (ci->efmodel == qfalse) 
		{
			//CG_CenterPrint( "QUAKE 3 MODEL TAUNT", SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH );// Shafe - Debug
			trap_S_StartSound (NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*taunt.wav" ) );
		}
		else
		{
			//CG_CenterPrint( "Elite Force MODEL TAUNT", SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH );// Shafe - Debug
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	case EV_MISSILE_MISS:
		...
		ByteToDir( es->eventParm, dir );
		CG_MissileHitWall( cent, es->weapon, 0, position, dir, IMPACTSOUND_DEFAULT );
		break;

	case EV_MISSILE_MISS_METAL:
		DEBUGNAME("EV_MISSILE_MISS_METAL");
		ByteToDir( es->eventParm, dir );
		CG_MissileHitWall( cent, es->weapon, 0, position, dir, IMPACTSOUND_METAL );
		break;
...
//unlagged - attack prediction #2
		...
		if ( es->clientNum == cg.predictedPlayerState.clientNum && 
			...
			if ( es->eventParm != 255 ) {
				ByteToDir( es->eventParm, dir );
				CG_MissileHitWall( cent, es->weapon, es->clientNum, position, dir, IMPACTSOUND_DEFAULT );
			}
