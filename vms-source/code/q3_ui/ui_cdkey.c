...
static void UI_CDKeyMenu_DrawKey( void *self ) {
	...
	if( val == 1 ) {
		UI_DrawProportionalString( 320, 376, "Ignore This, And Click The ACCEPT Button.", UI_CENTER|UI_SMALLFONT, color_yellow ); // Shafe - Trep - Need to Get Rid Of All This CD Key Shit
	}
	else if ( val == 0 ) {
...
static void UI_CDKeyMenu_Init( void ) {
	...
	cdkeyMenuInfo.cdkey.generic.name				= "IGNORE THE CD KEY, PRESS ACCEPT:"; // Shafe - Trep - Need To Fix This Up Better
