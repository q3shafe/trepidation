...
static void PM_Weapon( void ) {
	...
	if ( pm->cmd.buttons & BUTTON_USE_HOLDABLE ) 
	{
		...
		{ // Medkits are now holdable -Vincent			
...
// Shafe - Trep - Ladder Functions - Stolen But Credited ;)
/*
===================
PM_LadderMove()
...
void PmoveSingle (pmove_t *pmove) {
	...
	// Shafe - Trep - Alt Fire Code
	if ( !(pm->ps->pm_flags & PMF_RESPAWNED) && pm->ps->pm_type != PM_INTERMISSION && ( (pm->cmd.buttons & BUTTON_ATTACK) || (pm->cmd.buttons & 32) ) && pm->ps->ammo[ pm->ps->weapon ] ) { 
	...
	CheckLadder();	// Shafe - Trep - Ladder Support
