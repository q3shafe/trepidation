...
;864:
;865:	// Resetting Custom Speed and Gravity - Shafe - Dunno... 
;866:	self->r.svFlags &= ~SVF_CUSTOM_GRAVITY;
ADDRLP4 100
ADDRFP4 0
INDIRP4
CNSTI4 432
...
ASGNI4
line 867
;867:	self->r.svFlags &= ~SVF_CUSTOM_SPEED;
ADDRLP4 104
ADDRFP4 0
INDIRP4
CNSTI4 432
...
line 869
;868:
;869:	Cmd_Score_f( self );		// show scores
ADDRFP4 0
INDIRP4
ARGP4
ADDRGP4 Cmd_Score_f
...
pop
line 872
;870:	// send updated scores to any clients that are following this one,
;871:	// or they would get stale scoreboards
;872:	for ( i = 0 ; i < level.maxclients ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $380
...
line 879
;878:		}
;879:		if ( client->sess.sessionTeam != TEAM_SPECTATOR ) {
ADDRLP4 108
INDIRP4
CNSTI4 2552
ADDP4
...
line 882
;881:		}
;882:		if ( client->sess.spectatorClient == self->s.number ) {
ADDRLP4 108
INDIRP4
CNSTI4 2564
ADDP4
...
line 887
;886:
;887:	self->takedamage = qtrue;	// can still be gibbed
ADDRFP4 0
INDIRP4
CNSTI4 760
ADDP4
...
ASGNI4
line 891
;891:	self->r.contents = CONTENTS_CORPSE;
ADDRFP4 0
INDIRP4
CNSTI4 468
ADDP4
...
ASGNF4
line 895
;895:	LookAtKiller (self, inflictor, attacker);
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
...
line 897
;896:
;897:	VectorCopy( self->s.angles, self->client->ps.viewangles );
ADDRLP4 108
ADDRFP4 0
INDIRP4
ASGNP4
...
line 905
;902:
;903:	// don't allow respawn until the death anim is done
;904:	// g_forcerespawn may force spawning at some later time
;905:	self->client->respawnTime = level.time + 1700;  // This is the real line
ADDRFP4 0
INDIRP4
CNSTI4 524
ADDP4
...
;908:
;909:	// remove powerups
;910:	memset( self->client->ps.powerups, 0, sizeof(self->client->ps.powerups) );
ADDRFP4 0
INDIRP4
CNSTI4 524
ADDP4
...
line 912
;911:
;912:	if (meansOfDeath == MOD_HEADSHOT)
ADDRFP4 16
INDIRI4
CNSTI4 23
NEI4 $389
...
;913:	{
line 914
;914:			BroadCastSound("sound/misc/headshot.wav");
ADDRGP4 $391
ARGP4
ADDRGP4 BroadCastSound
CALLV
...
;917:
;918:
;919:	// never gib in a nodrop
;920:	if ( (self->health <= GIB_HEALTH && !(contents & CONTENTS_NODROP) && g_blood.integer && meansOfDeath != MOD_HEADSHOT) || meansOfDeath == MOD_SUICIDE) {
ADDRFP4 0
INDIRP4
CNSTI4 756
ADDP4
...
;938:		}
;939:
;940:		// for the no-blood option, we need to prevent the health
;941:		// from going to gib level
;942:		if ( self->health <= GIB_HEALTH ) {
ADDRFP4 0
INDIRP4
CNSTI4 756
ADDP4
...
ASGNI4
line 948
;947:			( ( self->client->ps.legsAnim & ANIM_TOGGLEBIT ) ^ ANIM_TOGGLEBIT ) | anim;
;948:		self->client->ps.torsoAnim = 
ADDRLP4 124
ADDRFP4 0
INDIRP4
CNSTI4 524
...
ASGNI4
line 951
;949:			( ( self->client->ps.torsoAnim & ANIM_TOGGLEBIT ) ^ ANIM_TOGGLEBIT ) | anim;
;950:
;951:		G_AddEvent( self, EV_DEATH1 + i, killer );
ADDRFP4 0
INDIRP4
ARGP4
ADDRGP4 $399
...
line 954
;952:
;953:		// Shafe - Trep Headshot //////////////////////////////////////////
;954:		if(meansOfDeath == MOD_HEADSHOT)
ADDRFP4 16
INDIRI4
CNSTI4 23
NEI4 $407
line 955
;955:			GibEntity_Headshot( self, killer );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 8
...
LABELV $408
line 961
;958:		// Shafe - Trep - End Headshot /////////////////////////////
;959:
;960:		// the body can still be gibbed
;961:		self->die = body_die;
ADDRFP4 0
INDIRP4
CNSTI4 740
ADDP4
...
line 964
;962:
;963:		// globally cycle through the different death animations
;964:		i = ( i + 1 ) % 3;
ADDRLP4 132
ADDRGP4 $399
ASGNP4
...
;969:		if (self->s.eFlags & EF_KAMIKAZE) {
;970:			Kamikaze_DeathTimer( self );
;971:		}
;972:#endif
;973:	}
LABELV $393
...
;986:================
;987:*/
;988:int CheckArmor (gentity_t *ent, int damage, int dflags)
;989:{
line 994
;990:	gclient_t	*client;
;991:	int			save;
;992:	int			count;
;993:
;994:	if (!damage)
...
line 1002
;1001:
;1002:	if (dflags & DAMAGE_NO_ARMOR)
ADDRFP4 8
INDIRI4
CNSTI4 2
BANDI4
...
;1004:
;1005:	// armor
;1006:	count = client->ps.stats[STAT_ARMOR];
ADDRLP4 8
ADDRLP4 4
INDIRP4
CNSTI4 196
...
ASGNI4
line 1007
;1007:	save = ceil( damage * ARMOR_PROTECTION );
CNSTF4 1059648963
ADDRFP4 4
INDIRI4
CVIF4 4
...
line 1014
;1013:
;1014:	client->ps.stats[STAT_ARMOR] -= save;
ADDRLP4 16
ADDRLP4 4
INDIRP4
CNSTI4 196
...
;1019:/*
;1020:================
;1021:RaySphereIntersections
;1022:================
;1023:*/
;1024:int RaySphereIntersections( vec3_t origin, float radius, vec3_t point, vec3_t dir, vec3_t intersections[2] ) {
line 1033
;1025:	float b, c, d, t;
;1026:
;1027:	//	| origin - (point + t * dir) | = radius
;1028:	//	a = dir[0]^2 + dir[1]^2 + dir[2]^2;
;1029:	//	b = 2 * (dir[0] * (point[0] - origin[0]) + dir[1] * (point[1] - origin[1]) + dir[2] * (point[2] - origin[2]));
;1030:	//	c = (point[0] - origin[0])^2 + (point[1] - origin[1])^2 + (point[2] - origin[2])^2 - radius^2;
;1031:
;1032:	// normalize dir so a = 1
;1033:	VectorNormalize(dir);
ADDRFP4 12
INDIRP4
ARGP4
...
pop
line 1034
;1034:	b = 2 * (dir[0] * (point[0] - origin[0]) + dir[1] * (point[1] - origin[1]) + dir[2] * (point[2] - origin[2]));
ADDRLP4 16
ADDRFP4 12
INDIRP4
ASGNP4
...
ASGNF4
line 1035
;1035:	c = (point[0] - origin[0]) * (point[0] - origin[0]) +
ADDRLP4 36
ADDRFP4 8
INDIRP4
ASGNP4
...
ASGNF4
line 1040
;1036:		(point[1] - origin[1]) * (point[1] - origin[1]) +
;1037:		(point[2] - origin[2]) * (point[2] - origin[2]) -
;1038:		radius * radius;
;1039:
;1040:	d = b * b - 4 * c;
ADDRLP4 8
...
ASGNF4
line 1043
;1043:		VectorMA(point, t, dir, intersections[0]);
ADDRFP4 16
INDIRP4
ADDRFP4 8
INDIRP4
...
ASGNF4
line 1045
;1045:		VectorMA(point, t, dir, intersections[1]);
ADDRFP4 16
INDIRP4
CNSTI4 12
ADDP4
...
ASGNF4
line 1050
;1050:		VectorMA(point, t, dir, intersections[0]);
ADDRFP4 16
INDIRP4
ADDRFP4 8
INDIRP4
...
;1057:/*
;1058:================
;1059:G_InvulnerabilityEffect
;1060:================
;1061:*/
;1062:int G_InvulnerabilityEffect( gentity_t *targ, vec3_t dir, vec3_t point, vec3_t impactpoint, vec3_t bouncedir ) {
;1063:	gentity_t	*impact;
;1064:	vec3_t		intersections[2], vec;
;1065:	int			n;
;1066:
;1067:	if ( !targ->client ) {
;1068:		return qfalse;
;1069:	}
;1070:	VectorCopy(dir, vec);
;1071:	VectorInverse(vec);
;1072:	// sphere model radius = 42 units
;1073:	n = RaySphereIntersections( targ->client->ps.origin, 42, point, vec, intersections);
;1074:	if (n > 0) {
;1075:		impact = G_TempEntity( targ->client->ps.origin, EV_INVUL_IMPACT );
;1076:		VectorSubtract(intersections[0], targ->client->ps.origin, vec);
;1077:		vectoangles(vec, impact->s.angles);
;1078:		impact->s.angles[0] += 90;
;1079:		if (impact->s.angles[0] > 360)
;1080:			impact->s.angles[0] -= 360;
;1081:		if ( impactpoint ) {
;1082:			VectorCopy( intersections[0], impactpoint );
;1083:		}
;1084:		if ( bouncedir ) {
;1085:			VectorCopy( vec, bouncedir );
;1086:			VectorNormalize( bouncedir );
;1087:		}
;1088:		return qtrue;
;1089:	}
;1090:	else {
...
;1097:T_Damage
;1098:
;1099:targ		entity that is being damaged
;1100:inflictor	entity that is causing the damage
;1101:attacker	entity that caused the inflictor to damage targ
;1102:	example: targ=monster, inflictor=rocket, attacker=player
;1103:
;1104:dir			direction of the attack for knockback
;1105:point		point at which the damage is being inflicted, used for headshots
;1106:damage		amount of damage being inflicted
;1107:knockback	force to be applied against targ as a result of the damage
;1108:
;1109:inflictor, attacker, dir, and point can be NULL for environmental effects
;1110:
;1111:dflags		these flags are used to control how T_Damage works
;1112:	DAMAGE_RADIUS			damage was indirect (from a nearby explosion)
;1113:	DAMAGE_NO_ARMOR			armor does not protect from this damage
;1114:	DAMAGE_NO_KNOCKBACK		do not affect velocity, just view angles
;1115:	DAMAGE_NO_PROTECTION	kills godmode, armor, everything
;1116:============
;1117:*/
;1118:
;1119:void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
;1120:			   vec3_t dir, vec3_t point, int damage, int dflags, int mod ) {
line 1138
;1121:	gclient_t	*client;
;1122:	int			take;
;1123:	int			save;
;1124:	int			asave;
;1125:	int			knockback;
;1126:	int			max;
;1127:	// Shafe - Trep - Headshot
;1128:	float		z_ratio;
;1129:	float		z_rel;
;1130:	int			height;
;1131:	float		targ_maxs2;
;1132:	// Shafe - Trep - End Headshot
;1133:
;1134:#ifdef MISSIONPACK
;1135:	vec3_t		bouncedir, impactpoint;
;1136:#endif
;1137:
;1138:	if (!targ->takedamage) {
ADDRFP4 0
INDIRP4
CNSTI4 760
ADDP4
...
;1140:	}
;1141:
;1142:	// the intermission has allready been qualified for, so don't
;1143:	// allow any extra scoring
;1144:	if ( level.intermissionQueued ) {
ADDRGP4 level+9136
INDIRI4
CNSTI4 0
EQI4 $428
...
;1146:	}
;1147:
;1148:	// Grapple hook cannot be used to damage a buildable
;1149:	if (targ->s.eType ==ET_BUILDABLE && mod == MOD_GRAPPLE && g_GrappleMode.integer == 1)
ADDRFP4 0
INDIRP4
CNSTI4 4
ADDP4
...
;1158:	if ( targ->client && mod != MOD_JUICED) {
;1159:		if ( targ->client->invulnerabilityTime > level.time) {
;1160:			if ( dir && point ) {
;1161:				G_InvulnerabilityEffect( targ, dir, point, impactpoint, bouncedir );
;1162:			}
;1163:			return;
;1164:		}
;1165:	}
...
NEU4 $434
line 1168
;1168:		inflictor = &g_entities[ENTITYNUM_WORLD];
ADDRFP4 4
ADDRGP4 g_entities+944328
ASGNP4
line 1169
...
NEU4 $437
line 1171
;1171:		attacker = &g_entities[ENTITYNUM_WORLD];
ADDRFP4 8
ADDRGP4 g_entities+944328
ASGNP4
line 1172
...
;1174:	
;1175:
;1176:	// shootable doors / buttons don't actually have any health
;1177:	if ( targ->s.eType == ET_MOVER ) {
ADDRLP4 40
CNSTI4 4
ASGNI4
ADDRFP4 0
...
NEI4 $440
line 1178
;1178:		if ( targ->use && targ->moverState == MOVER_POS1 || targ->moverState == ROTATOR_POS1) {
ADDRLP4 44
ADDRFP4 0
INDIRP4
ASGNP4
...
LABELV $444
line 1179
;1179:			targ->use( targ, inflictor, attacker );
ADDRLP4 48
ADDRFP4 0
INDIRP4
ASGNP4
...
;1182:	}
;1183:
;1184:	// If we shot a breakable item subtract the damage from its health and try to break it
;1185: 	if ( targ->s.eType == ET_BREAKABLE ) {
ADDRFP4 0
INDIRP4
CNSTI4 4
ADDP4
...
ASGNI4
line 1187
;1187: 		G_BreakGlass( targ, point, mod );
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 16
...
;1191:	if( g_gametype.integer == GT_OBELISK && CheckObeliskAttack( targ, attacker ) ) {
;1192:		return;
;1193:	}
;1194:#endif
;1195:	// reduce damage by the attacker's handicap value
;1196:	// unless they are rocket jumping
;1197:	if ( attacker->client && attacker != targ ) {
ADDRLP4 44
ADDRFP4 8
INDIRP4
ASGNP4
...
EQU4 $448
line 1198
;1198:		max = attacker->client->ps.stats[STAT_MAX_HEALTH];
ADDRLP4 20
ADDRFP4 8
INDIRP4
CNSTI4 524
...
;1200:		if( bg_itemlist[attacker->client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_GUARD ) {
;1201:			max /= 2;
;1202:		}
;1203:#endif
;1204:		damage = damage * max / 100;
ADDRFP4 20
ADDRFP4 20
INDIRI4
ADDRLP4 20
...
NEU4 $454
line 1216
;1216:		dflags |= DAMAGE_NO_KNOCKBACK;
ADDRFP4 24
ADDRFP4 24
INDIRI4
CNSTI4 4
...
line 1221
;1220:
;1221:	knockback = damage;
ADDRLP4 4
ADDRFP4 20
INDIRI4
ASGNI4
line 1222
;1222:	if ( knockback > 200 ) {
ADDRLP4 4
INDIRI4
CNSTI4 200
LEI4 $456
...
LABELV $456
line 1225
;1225:	if ( targ->flags & FL_NO_KNOCKBACK ) {
ADDRFP4 0
INDIRP4
CNSTI4 544
ADDP4
...
LABELV $458
line 1228
;1228:	if ( dflags & DAMAGE_NO_KNOCKBACK ) {
ADDRFP4 24
INDIRI4
CNSTI4 4
BANDI4
...
line 1233
;1231:
;1232:	// figure momentum add, even if the damage won't be taken
;1233:	if ( knockback && targ->client ) {
ADDRLP4 4
INDIRI4
CNSTI4 0
EQI4 $462
...
line 1239
;1238:
;1239:		VectorScale (dir, g_knockback.value * (float)knockback / mass, kvel);
ADDRLP4 64
ADDRFP4 12
INDIRP4
ASGNP4
...
ASGNF4
line 1240
;1240:		VectorAdd (targ->client->ps.velocity, kvel, targ->client->ps.velocity);
ADDRLP4 76
ADDRFP4 0
INDIRP4
CNSTI4 524
...
line 1244
;1241:
;1242:		// set the timer so that the other client can't cancel
;1243:		// out the movement immediately
;1244:		if ( !targ->client->ps.pm_time ) {
ADDRFP4 0
INDIRP4
CNSTI4 524
ADDP4
...
ASGNI4
line 1255
;1255:			targ->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
ADDRLP4 92
ADDRFP4 0
INDIRP4
CNSTI4 524
...
line 1260
;1258:
;1259:	// check for completely getting out of the damage
;1260:	if ( !(dflags & DAMAGE_NO_PROTECTION) ) {
ADDRFP4 24
INDIRI4
CNSTI4 8
BANDI4
...
line 1267
;1261:
;1262:		// if TF_NO_FRIENDLY_FIRE is set, don't do damage to the target
;1263:		// if the attacker was on the same team
;1264:#ifdef MISSIONPACK
;1265:		if ( mod != MOD_JUICED && targ != attacker && !(dflags & DAMAGE_NO_TEAM_PROTECTION) && OnSameTeam (targ, attacker)  ) {
;1266:#else	
;1267:		if ( targ != attacker && OnSameTeam (targ, attacker)  ) {
ADDRLP4 48
ADDRFP4 0
INDIRP4
ASGNP4
...
line 1269
;1268:#endif
;1269:			if ( !g_friendlyFire.integer ) {
...
;1274:
;1275:	/*
;1276:	if (mod == MOD_GRAPPLE && g_GrappleMode.integer == 2)
;1277:	{
;1278:		//targ->client->ps.speed = 0;
;1279:		targ->immobilized == qtrue;
;1280:		return;
;1281:	}
;1282:	*/
;1283:
;1284:	
;1285:	// No Team Killing Of MC -  Make this a cvar
;1286:	if ((targ->s.eType ==ET_BUILDABLE) && (targ->s.team == attacker->client->sess.sessionTeam))
ADDRLP4 60
ADDRFP4 0
INDIRP4
ASGNP4
...
;1287:	{
line 1288
;1288:		if ((!strcmp(targ->classname, "mc")) && (g_PCTeamkills.integer == 0))
ADDRFP4 0
INDIRP4
CNSTI4 532
ADDP4
...
;1293:
;1294:	
;1295:	if ((targ->s.eType ==ET_BUILDABLE) && (targ->health < 1000))
ADDRLP4 64
ADDRFP4 0
INDIRP4
ASGNP4
...
;1296:	{
line 1297
;1297:		if (!strcmp(targ->classname, "mc"))
ADDRFP4 0
INDIRP4
CNSTI4 532
ADDP4
...
;1298:		{	
line 1299
;1299:				TeamCP("^9Power Core is Under Attack!",targ->s.team);
ADDRGP4 $494
ARGP4
ADDRFP4 0
INDIRP4
...
;1302:	
;1303:	// Debug
;1304:	if (trep_debug.integer) { G_Printf("Class : %s Taking Damage\n", targ->classname ); }
...
;1305:	
;1306:
;1307:#ifdef MISSIONPACK
;1308:		if (mod == MOD_PROXIMITY_MINE) {
;1309:			if (inflictor && inflictor->parent && OnSameTeam(targ, inflictor->parent)) {
;1310:				return;
;1311:			}
;1312:			if (targ == attacker) {
;1313:				return;
;1314:			}
;1315:		}
;1316:#endif
;1317:
;1318:		// check for godmode
;1319:		if ( targ->flags & FL_GODMODE ) {
ADDRFP4 0
INDIRP4
CNSTI4 544
ADDP4
...
line 1326
;1323:
;1324:	// battlesuit protects from all radius damage (but takes knockback)
;1325:	// and protects 50% against all damage
;1326:	if ( client && client->ps.powerups[PW_BATTLESUIT] ) {
ADDRLP4 0
INDIRP4
CVPU4 4
CNSTU4 0
...
EQI4 $501
line 1327
;1327:		G_AddEvent( targ, EV_POWERUP_BATTLESUIT, 0 );
ADDRFP4 0
INDIRP4
ARGP4
CNSTI4 64
...
pop
line 1328
;1328:		if ( ( dflags & DAMAGE_RADIUS ) || ( mod == MOD_FALLING ) ) {
ADDRFP4 24
INDIRI4
CNSTI4 1
BANDI4
...
line 1335
;1333:
;1334:	// add to the attacker's hit counter (if the target isn't a general entity like a prox mine)
;1335:	if ( attacker->client && targ != attacker && targ->health > 0
ADDRLP4 52
ADDRFP4 8
INDIRP4
ASGNP4
...
EQI4 $506
line 1337
;1336:			&& targ->s.eType != ET_MISSILE
;1337:			&& targ->s.eType != ET_GENERAL) {
line 1338
;1338:		if ( OnSameTeam( targ, attacker ) ) {
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 8
...
EQI4 $508
line 1339
;1339:			attacker->client->ps.persistant[PERS_HITS]--;
ADDRLP4 72
ADDRFP4 8
INDIRP4
CNSTI4 524
...
LABELV $508
line 1341
;1341:			attacker->client->ps.persistant[PERS_HITS]++;
ADDRLP4 72
ADDRFP4 8
INDIRP4
CNSTI4 524
...
LABELV $509
line 1343
;1343:		attacker->client->ps.persistant[PERS_ATTACKEE_ARMOR] = (targ->health<<8)|(client->ps.stats[STAT_ARMOR]);
ADDRFP4 8
INDIRP4
CNSTI4 524
ADDP4
...
line 1348
;1345:
;1346:	// always give half damage if hurting self
;1347:	// calculated after knockback, so rocket jumping works
;1348:	if ( targ == attacker) {
ADDRFP4 0
INDIRP4
CVPU4 4
ADDRFP4 8
...
line 1359
;1357:
;1358:	// save some from armor
;1359:	asave = CheckArmor (targ, take, dflags);
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 8
...
line 1362
;1361:
;1362:	if ( g_debugDamage.integer ) {
ADDRGP4 g_debugDamage+12
INDIRI4
CNSTI4 0
EQI4 $514
line 1363
;1363:		G_Printf( "%i: client:%i health:%i damage:%i armor:%i\n", level.time, targ->s.number,
ADDRGP4 $517
ARGP4
ADDRGP4 level+32
INDIRI4
...
pop
line 1365
;1364:			targ->health, take, asave );
;1365:	}
LABELV $514
line 1370
;1366:
;1367:	// add to the damage inflicted on a player this frame
;1368:	// the total will be turned into screen blends and view angle kicks
;1369:	// at the end of the frame
;1370:	if ( client ) {
ADDRLP4 0
INDIRP4
CVPU4 4
...
EQU4 $521
line 1372
;1372:			client->ps.persistant[PERS_ATTACKER] = attacker->s.number;
ADDRLP4 0
INDIRP4
CNSTI4 272
ADDP4
...
LABELV $521
line 1374
;1374:			client->ps.persistant[PERS_ATTACKER] = ENTITYNUM_WORLD;
ADDRLP4 0
INDIRP4
CNSTI4 272
ADDP4
...
LABELV $522
line 1376
;1376:		client->damage_armor += asave;
ADDRLP4 72
ADDRLP4 0
INDIRP4
CNSTI4 2612
...
ASGNI4
line 1378
;1378:		client->damage_knockback += knockback;
ADDRLP4 80
ADDRLP4 0
INDIRP4
CNSTI4 2620
...
EQU4 $523
line 1380
;1380:			VectorCopy ( dir, client->damage_from );
ADDRLP4 0
INDIRP4
CNSTI4 2624
ADDP4
...
ASGNB 12
line 1381
;1381:			client->damage_fromWorld = qfalse;
ADDRLP4 0
INDIRP4
CNSTI4 2636
ADDP4
...
LABELV $523
line 1383
;1383:			VectorCopy ( targ->r.currentOrigin, client->damage_from );
ADDRLP4 0
INDIRP4
CNSTI4 2624
ADDP4
...
ASGNB 12
line 1384
;1384:			client->damage_fromWorld = qtrue;
ADDRLP4 0
INDIRP4
CNSTI4 2636
ADDP4
...
line 1392
;1387:
;1388:	// See if it's the player hurting the emeny flag carrier
;1389:#ifdef MISSIONPACK
;1390:	if( g_gametype.integer == GT_CTF || g_gametype.integer == GT_1FCTF ) {
;1391:#else	
;1392:	if( g_gametype.integer == GT_CTF) {
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 4
NEI4 $525
line 1394
;1393:#endif
;1394:		Team_CheckHurtCarrier(targ, attacker);
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 8
...
EQU4 $528
line 1399
;1398:		// set the last client who damaged the target
;1399:		targ->client->lasthurt_client = attacker->s.number;
ADDRFP4 0
INDIRP4
CNSTI4 524
ADDP4
...
ASGNI4
line 1400
;1400:		targ->client->lasthurt_mod = mod;
ADDRFP4 0
INDIRP4
CNSTI4 524
ADDP4
...
line 1404
;1402:
;1403:	// Shafe - Trep - Headshots
;1404:	if (targ->client && attacker->client && targ->health > 0)
ADDRLP4 72
ADDRFP4 0
INDIRP4
ASGNP4
...
;1405:	{   
line 1407
;1406:		// let's say only railgun can do head shots
;1407:		if(inflictor->s.weapon==WP_RAILGUN){
ADDRFP4 4
INDIRP4
CNSTI4 192
ADDP4
...
line 1411
;1409:	
;1410:			// handling crouching
;1411:			if(targ->client->ps.pm_flags & PMF_DUCKED){
ADDRFP4 0
INDIRP4
CNSTI4 524
ADDP4
...
EQI4 $534
line 1412
;1412:				height = (abs(targ->r.mins[2]) + targ_maxs2)*(0.75);
ADDRFP4 0
INDIRP4
CNSTI4 452
ADDP4
...
line 1415
;1414:			else
;1415:				height = abs(targ->r.mins[2]) + targ_maxs2; 
ADDRFP4 0
INDIRP4
CNSTI4 452
ADDP4
...
line 1420
;1416:				
;1417:			// project the z component of point 
;1418:			// onto the z component of the model's origin
;1419:			// this results in the z component from the origin at 0
;1420:			z_rel = point[2] - targ->r.currentOrigin[2] + abs(targ->r.mins[2]);
ADDRLP4 88
ADDRFP4 0
INDIRP4
ASGNP4
...
line 1423
;1422:		
;1423:			if (z_ratio > 0.90){
ADDRLP4 24
INDIRF4
CNSTF4 1063675494
LEF4 $536
line 1424
;1424:				take=9999; // head shot is a sure kill
ADDRLP4 8
CNSTI4 9999
ASGNI4
line 1425
;1425:				mod=MOD_HEADSHOT;
ADDRFP4 28
CNSTI4 23
ASGNI4
line 1426
...
LABELV $530
line 1433
;1429:	// Shafe - Trep - End Headshot Code
;1430:
;1431:
;1432:	// do the damage
;1433:	if (take) {
...
EQI4 $538
line 1434
;1434:		targ->health = targ->health - take;
ADDRLP4 84
ADDRFP4 0
INDIRP4
CNSTI4 756
...
EQU4 $540
line 1436
;1436:			targ->client->ps.stats[STAT_HEALTH] = targ->health;
ADDRLP4 88
ADDRFP4 0
INDIRP4
ASGNP4
...
EQU4 $544
line 1441
;1441:				targ->flags |= FL_NO_KNOCKBACK;
ADDRLP4 88
ADDRFP4 0
INDIRP4
CNSTI4 544
...
ASGNP4
line 1447
;1447:			targ->die (targ, inflictor, attacker, take, mod);
ADDRLP4 92
ADDRFP4 0
INDIRP4
ASGNP4
...
EQU4 $548
line 1450
;1450:			targ->pain (targ, attacker, take);
ADDRLP4 88
ADDRFP4 0
INDIRP4
ASGNP4
...
;1459:CanDamage
;1460:
;1461:Returns qtrue if the inflictor can directly damage the target.  Used for
;1462:explosions and melee attacks.
;1463:============
;1464:*/
;1465:qboolean CanDamage (gentity_t *targ, vec3_t origin) {
line 1472
;1466:	vec3_t	dest;
;1467:	trace_t	tr;
;1468:	vec3_t	midpoint;
;1469:
;1470:	// use the midpoint of the bounds instead of the origin, because
;1471:	// bmodels may have their origin is 0,0,0
;1472:	VectorAdd (targ->r.absmin, targ->r.absmax, midpoint);
ADDRLP4 80
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNF4
line 1473
;1473:	VectorScale (midpoint, 0.5, midpoint);
ADDRLP4 88
CNSTF4 1056964608
ASGNF4
ADDRLP4 12
...
line 1475
;1474:
;1475:	VectorCopy (midpoint, dest);
ADDRLP4 0
ADDRLP4 12
INDIRB
ASGNB 12
line 1476
;1476:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
pop
line 1477
;1477:	if (tr.fraction == 1.0 || tr.entityNum == targ->s.number)
ADDRLP4 24+8
INDIRF4
CNSTF4 1065353216
EQF4 $561
...
line 1482
;1479:
;1480:	// this should probably check in the plane of projection, 
;1481:	// rather than in world coordinate, and also include Z
;1482:	VectorCopy (midpoint, dest);
ADDRLP4 0
ADDRLP4 12
INDIRB
ASGNB 12
...
ASGNF4
line 1485
;1485:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1492
;1492:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1499
;1499:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1506
;1506:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
;1518:============
;1519:*/
;1520:qboolean G_RadiusDamage ( vec3_t origin, gentity_t *attacker, float damage, float radius,
;1521:					 gentity_t *ignore, int mod) {
line 1530
;1522:	float		points, dist;
;1523:	gentity_t	*ent;
;1524:	int			entityList[MAX_GENTITIES];
;1525:	int			numListedEntities;
;1526:	vec3_t		mins, maxs;
;1527:	vec3_t		v;
;1528:	vec3_t		dir;
;1529:	int			i, e;
;1530:	qboolean	hitClient = qfalse;
ADDRLP4 4168
CNSTI4 0
ASGNI4
line 1532
...
line 1541
;1540:
;1541:	numListedEntities = trap_EntitiesInBox( mins, maxs, entityList, MAX_GENTITIES );
ADDRLP4 4144
ARGP4
ADDRLP4 4156
ARGP4
...
line 1543
;1542:
;1543:	for ( e = 0 ; e < numListedEntities ; e++ ) {
ADDRLP4 20
CNSTI4 0
ASGNI4
ADDRGP4 $588
...
LABELV $585
line 1544
;1544:		ent = &g_entities[entityList[ e ]];
ADDRLP4 4
CNSTI4 924
ADDRLP4 20
INDIRI4
...
line 1552
;1550:
;1551:		// find the distance from the edge of the bounding box
;1552:		for ( i = 0 ; i < 3 ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $593
line 1553
;1553:			if ( origin[i] < ent->r.absmin[i] ) {
ADDRLP4 4176
ADDRLP4 0
INDIRI4
CNSTI4 2
...
GEF4 $597
line 1554
;1554:				v[i] = ent->r.absmin[i] - origin[i];
ADDRLP4 4180
ADDRLP4 0
INDIRI4
CNSTI4 2
...
ASGNF4
line 1555
;1555:			} else if ( origin[i] > ent->r.absmax[i] ) {
ADDRGP4 $598
JUMPV
LABELV $597
ADDRLP4 4180
...
LEF4 $599
line 1556
;1556:				v[i] = origin[i] - ent->r.absmax[i];
ADDRLP4 4184
ADDRLP4 0
INDIRI4
CNSTI4 2
...
;1565:		}
;1566:
;1567:		points = damage * ( 1.0 - dist / radius );
ADDRLP4 40
ADDRFP4 8
INDIRF4
CNSTF4 1065353216
...
EQI4 $603
line 1570
;1570:			if( LogAccuracyHit( ent, attacker ) ) {
ADDRLP4 4
INDIRP4
ARGP4
ADDRFP4 4
...
LABELV $605
line 1573
;1573:			VectorSubtract (ent->r.currentOrigin, origin, dir);
ADDRLP4 4192
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNF4
line 1576
;1574:			// push the center of mass higher than the origin so players
;1575:			// get knocked into the air more
;1576:			dir[2] += 24;
ADDRLP4 28+8
ADDRLP4 28+8
INDIRF4
...
ASGNF4
line 1577
;1577:			G_Damage (ent, NULL, attacker, dir, origin, (int)points, DAMAGE_RADIUS, mod);
ADDRLP4 4
INDIRP4
ARGP4
CNSTP4 0
