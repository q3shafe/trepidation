...
line 1415
;1414:		
;1415:		if ( ( dflags & DAMAGE_RADIUS ) || ( mod == MOD_FALLING ) ) {
ADDRFP4 24
INDIRI4
CNSTI4 1
BANDI4
...
;1423:
;1424:
;1425:	// add to the attacker's hit counter (if the target isn't a general entity like a prox mine)
;1426:	if ( attacker->client && targ != attacker && targ->health > 0
ADDRLP4 56
ADDRFP4 8
INDIRP4
ASGNP4
...
EQI4 $546
line 1429
;1427:			&& targ->s.eType != ET_MISSILE
;1428:			&& targ->s.eType != ET_GENERAL
;1429:			&& targ->s.eType != ET_BUILDABLE ) {
line 1430
;1430:		if ( OnSameTeam( targ, attacker ) ) {
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 8
...
EQI4 $548
line 1431
;1431:			attacker->client->ps.persistant[PERS_HITS]--;
ADDRLP4 76
ADDRFP4 8
INDIRP4
CNSTI4 524
...
LABELV $548
line 1433
;1433:			attacker->client->ps.persistant[PERS_HITS]++;
ADDRLP4 76
ADDRFP4 8
INDIRP4
CNSTI4 524
...
LABELV $549
line 1435
;1435:		attacker->client->ps.persistant[PERS_ATTACKEE_ARMOR] = (targ->health<<8)|(client->ps.stats[STAT_ARMOR]);
ADDRFP4 8
INDIRP4
CNSTI4 524
ADDP4
...
line 1440
;1437:
;1438:	// always give half damage if hurting self
;1439:	// calculated after knockback, so rocket jumping works
;1440:	if ( targ == attacker) {
ADDRFP4 0
INDIRP4
CVPU4 4
ADDRFP4 8
...
line 1451
;1449:
;1450:	// save some from armor
;1451:	asave = CheckArmor (targ, take, dflags);
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 8
...
line 1454
;1453:
;1454:	if ( g_debugDamage.integer ) {
ADDRGP4 g_debugDamage+12
INDIRI4
CNSTI4 0
EQI4 $554
line 1455
;1455:		G_Printf( "%i: client:%i health:%i damage:%i armor:%i\n", level.time, targ->s.number,
ADDRGP4 $557
ARGP4
ADDRGP4 level+32
INDIRI4
...
pop
line 1457
;1456:			targ->health, take, asave );
;1457:	}
LABELV $554
line 1462
;1458:
;1459:	// add to the damage inflicted on a player this frame
;1460:	// the total will be turned into screen blends and view angle kicks
;1461:	// at the end of the frame
;1462:	if ( client ) {
ADDRLP4 0
INDIRP4
CVPU4 4
...
EQU4 $561
line 1464
;1464:			client->ps.persistant[PERS_ATTACKER] = attacker->s.number;
ADDRLP4 0
INDIRP4
CNSTI4 272
ADDP4
...
LABELV $561
line 1466
;1466:			client->ps.persistant[PERS_ATTACKER] = ENTITYNUM_WORLD;
ADDRLP4 0
INDIRP4
CNSTI4 272
ADDP4
...
LABELV $562
line 1468
;1468:		client->damage_armor += asave;
ADDRLP4 76
ADDRLP4 0
INDIRP4
CNSTI4 2628
...
ASGNI4
line 1470
;1470:		client->damage_knockback += knockback;
ADDRLP4 84
ADDRLP4 0
INDIRP4
CNSTI4 2636
...
EQU4 $563
line 1472
;1472:			VectorCopy ( dir, client->damage_from );
ADDRLP4 0
INDIRP4
CNSTI4 2640
ADDP4
...
ASGNB 12
line 1473
;1473:			client->damage_fromWorld = qfalse;
ADDRLP4 0
INDIRP4
CNSTI4 2652
ADDP4
...
LABELV $563
line 1475
;1475:			VectorCopy ( targ->r.currentOrigin, client->damage_from );
ADDRLP4 0
INDIRP4
CNSTI4 2640
ADDP4
...
ASGNB 12
line 1476
;1476:			client->damage_fromWorld = qtrue;
ADDRLP4 0
INDIRP4
CNSTI4 2652
ADDP4
...
line 1484
;1479:
;1480:	// See if it's the player hurting the emeny flag carrier
;1481:#ifdef MISSIONPACK
;1482:	if( g_gametype.integer == GT_CTF || g_gametype.integer == GT_1FCTF ) {
;1483:#else	
;1484:	if( g_gametype.integer == GT_CTF) {
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 4
NEI4 $565
line 1486
;1485:#endif
;1486:		Team_CheckHurtCarrier(targ, attacker);
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 8
...
EQU4 $568
line 1491
;1490:		// set the last client who damaged the target
;1491:		targ->client->lasthurt_client = attacker->s.number;
ADDRFP4 0
INDIRP4
CNSTI4 524
ADDP4
...
ASGNI4
line 1492
;1492:		targ->client->lasthurt_mod = mod;
ADDRFP4 0
INDIRP4
CNSTI4 524
ADDP4
...
line 1496
;1494:
;1495:	// Shafe - Trep - Headshots
;1496:	if (targ->client && attacker->client && targ->health > 0)
ADDRLP4 76
ADDRFP4 0
INDIRP4
ASGNP4
...
;1497:	{   
line 1499
;1498:		// let's say only railgun can do head shots
;1499:		if(inflictor->s.weapon==WP_RAILGUN){
ADDRFP4 4
INDIRP4
CNSTI4 192
ADDP4
...
line 1503
;1501:	
;1502:			// handling crouching
;1503:			if(targ->client->ps.pm_flags & PMF_DUCKED){
ADDRFP4 0
INDIRP4
CNSTI4 524
ADDP4
...
EQI4 $574
line 1504
;1504:				height = (abs(targ->r.mins[2]) + targ_maxs2)*(0.75);
ADDRFP4 0
INDIRP4
CNSTI4 452
ADDP4
...
line 1507
;1506:			else
;1507:				height = abs(targ->r.mins[2]) + targ_maxs2; 
ADDRFP4 0
INDIRP4
CNSTI4 452
ADDP4
...
line 1512
;1508:				
;1509:			// project the z component of point 
;1510:			// onto the z component of the model's origin
;1511:			// this results in the z component from the origin at 0
;1512:			z_rel = point[2] - targ->r.currentOrigin[2] + abs(targ->r.mins[2]);
ADDRLP4 92
ADDRFP4 0
INDIRP4
ASGNP4
...
line 1515
;1514:		
;1515:			if (z_ratio > 0.90){
ADDRLP4 24
INDIRF4
CNSTF4 1063675494
LEF4 $576
line 1516
;1516:				take=9999; // head shot is a sure kill
ADDRLP4 8
CNSTI4 9999
ASGNI4
line 1517
;1517:				mod=MOD_HEADSHOT;
ADDRFP4 28
CNSTI4 23
ASGNI4
line 1518
...
LABELV $570
line 1525
;1521:	// Shafe - Trep - End Headshot Code
;1522:
;1523:
;1524:	// do the damage
;1525:	if (take) {
...
EQI4 $578
line 1526
;1526:		targ->health = targ->health - take;
ADDRLP4 88
ADDRFP4 0
INDIRP4
CNSTI4 756
...
EQU4 $580
line 1528
;1528:			targ->client->ps.stats[STAT_HEALTH] = targ->health;
ADDRLP4 92
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNI4
line 1529
;1529:			trap_SendServerCommand(client->ps.clientNum, "+greset");
ADDRLP4 0
INDIRP4
CNSTI4 140
ADDP4
...
EQU4 $585
line 1534
;1534:				targ->flags |= FL_NO_KNOCKBACK;
ADDRLP4 92
ADDRFP4 0
INDIRP4
CNSTI4 544
...
ASGNP4
line 1540
;1540:			targ->die (targ, inflictor, attacker, take, mod);
ADDRLP4 96
ADDRFP4 0
INDIRP4
ASGNP4
...
EQU4 $589
line 1543
;1543:			targ->pain (targ, attacker, take);
ADDRLP4 92
ADDRFP4 0
INDIRP4
ASGNP4
...
;1552:CanDamage
;1553:
;1554:Returns qtrue if the inflictor can directly damage the target.  Used for
;1555:explosions and melee attacks.
;1556:============
;1557:*/
;1558:qboolean CanDamage (gentity_t *targ, vec3_t origin) {
line 1565
;1559:	vec3_t	dest;
;1560:	trace_t	tr;
;1561:	vec3_t	midpoint;
;1562:
;1563:	// use the midpoint of the bounds instead of the origin, because
;1564:	// bmodels may have their origin is 0,0,0
;1565:	VectorAdd (targ->r.absmin, targ->r.absmax, midpoint);
ADDRLP4 80
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNF4
line 1566
;1566:	VectorScale (midpoint, 0.5, midpoint);
ADDRLP4 88
CNSTF4 1056964608
ASGNF4
ADDRLP4 12
...
line 1568
;1567:
;1568:	VectorCopy (midpoint, dest);
ADDRLP4 0
ADDRLP4 12
INDIRB
ASGNB 12
line 1569
;1569:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
pop
line 1570
;1570:	if (tr.fraction == 1.0 || tr.entityNum == targ->s.number)
ADDRLP4 24+8
INDIRF4
CNSTF4 1065353216
EQF4 $602
...
line 1575
;1572:
;1573:	// this should probably check in the plane of projection, 
;1574:	// rather than in world coordinate, and also include Z
;1575:	VectorCopy (midpoint, dest);
ADDRLP4 0
ADDRLP4 12
INDIRB
ASGNB 12
...
ASGNF4
line 1578
;1578:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1585
;1585:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1592
;1592:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1599
;1599:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 24
ARGP4
ADDRFP4 4
INDIRP4
...
;1611:============
;1612:*/
;1613:qboolean G_RadiusDamage ( vec3_t origin, gentity_t *attacker, float damage, float radius,
;1614:					 gentity_t *ignore, int mod) {
line 1623
;1615:	float		points, dist;
;1616:	gentity_t	*ent;
;1617:	int			entityList[MAX_GENTITIES];
;1618:	int			numListedEntities;
;1619:	vec3_t		mins, maxs;
;1620:	vec3_t		v;
;1621:	vec3_t		dir;
;1622:	int			i, e;
;1623:	qboolean	hitClient = qfalse;
ADDRLP4 4168
CNSTI4 0
ASGNI4
line 1625
...
line 1634
;1633:
;1634:	numListedEntities = trap_EntitiesInBox( mins, maxs, entityList, MAX_GENTITIES );
ADDRLP4 4144
ARGP4
ADDRLP4 4156
ARGP4
...
line 1636
;1635:
;1636:	for ( e = 0 ; e < numListedEntities ; e++ ) {
ADDRLP4 20
CNSTI4 0
ASGNI4
ADDRGP4 $629
...
LABELV $626
line 1637
;1637:		ent = &g_entities[entityList[ e ]];
ADDRLP4 4
CNSTI4 924
ADDRLP4 20
INDIRI4
...
line 1645
;1643:
;1644:		// find the distance from the edge of the bounding box
;1645:		for ( i = 0 ; i < 3 ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $634
line 1646
;1646:			if ( origin[i] < ent->r.absmin[i] ) {
ADDRLP4 4176
ADDRLP4 0
INDIRI4
CNSTI4 2
...
GEF4 $638
line 1647
;1647:				v[i] = ent->r.absmin[i] - origin[i];
ADDRLP4 4180
ADDRLP4 0
INDIRI4
CNSTI4 2
...
ASGNF4
line 1648
;1648:			} else if ( origin[i] > ent->r.absmax[i] ) {
ADDRGP4 $639
JUMPV
LABELV $638
ADDRLP4 4180
...
LEF4 $640
line 1649
;1649:				v[i] = origin[i] - ent->r.absmax[i];
ADDRLP4 4184
ADDRLP4 0
INDIRI4
CNSTI4 2
...
;1658:		}
;1659:
;1660:		points = damage * ( 1.0 - dist / radius );
ADDRLP4 40
ADDRFP4 8
INDIRF4
CNSTF4 1065353216
...
EQI4 $644
line 1663
;1663:			if( LogAccuracyHit( ent, attacker ) ) {
ADDRLP4 4
INDIRP4
ARGP4
ADDRFP4 4
...
LABELV $646
line 1666
;1666:			VectorSubtract (ent->r.currentOrigin, origin, dir);
ADDRLP4 4192
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNF4
line 1669
;1667:			// push the center of mass higher than the origin so players
;1668:			// get knocked into the air more
;1669:			dir[2] += 24;
ADDRLP4 28+8
ADDRLP4 28+8
INDIRF4
...
;1670:
;1671:
;1672:			G_Damage (ent, NULL, attacker, dir, origin, (int)points, DAMAGE_RADIUS, mod);
ADDRLP4 4
INDIRP4
ARGP4
CNSTP4 0
...
pop
line 1694
;1673:			// Guass Rifle Jumping -- This is causing the problems
;1674:			/*
;1675:			if (g_instagib.integer == 1)
;1676:			{
;1677:				// Is it a rocket jump?
;1678:
;1679:				if (ent == attacker)
;1680:				{
;1681:					G_Damage (ent, NULL, attacker, dir, origin, (int)points, DAMAGE_RADIUS, mod);
;1682:				} 
;1683:				else
;1684:				{			
;1685:					G_Damage (ent, NULL, attacker, 0, origin, (int)points, DAMAGE_RADIUS, mod);
;1686:				}
;1687:			} 
;1688:			else 
;1689:			{
